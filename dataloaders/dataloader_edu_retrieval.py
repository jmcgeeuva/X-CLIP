...
class Edu_DataLoader(Dataset):
    def __init__(
        ...
        video_dict = {}
        caption_dict = {}
        for class_dir in os.listdir(self.features_path):
            tag = class_dir.split('/')[-1]
            class_path = os.path.join(self.features_path, class_dir)
            for video_file in [file for file in os.listdir(class_path)]:
                video_path = os.path.join(class_path, video_file)
                video_id_=f'{tag}_{int(video_file):04d}'
                frames = [file for file in os.listdir(video_path) if 'jpg' in file]
                if len(frames) == 0 or not os.path.exists(os.path.join(video_path, 'new_annotation.json')):
                    continue
                video_dict[video_id_] = video_path
                # with open(os.path.join(video_path, 'new_annotation.json'), 'w') as f:
                #     json_dict = json.load(f)
                # frames = json_dict["frames"].keys()
                # Save the video path
                times = len(frames) # number of frames in the clip
                clip_length = 5
                gap = 1

                times = [start for start in range(0, times, clip_length + gap) if start + clip_length <= times]

                # each video is split into 5-second temporal chunks
                # average the points from each annotator
                start_times = [t_ for t_ in times]
                start_ = np.mean(start_times) * 5
                end_list = [t_ - 1 for i, t_ in enumerate(times) if i != 0]
                end_list.append(start_times[len(start_times)-1]+clip_length)
                end_ = (np.mean(end_list) + 1) * 5
                if video_id_ in caption_dict:
                    caption_dict[video_id_]["start"].append(start_)
                    caption_dict[video_id_]["end"].append(end_)
                else:
                    caption_dict[video_id_] = {}
                    caption_dict[video_id_]["start"] = [start_]
                    caption_dict[video_id_]["end"] = [end_]
    ...
    def _get_text(self, video_id, caption, sub_id):
        k = 1
        r_ind = [sub_id]

        starts = np.zeros(k, dtype=np.longlong)
        ends = np.zeros(k, dtype=np.longlong)
        ...
        annotations_file = os.path.join(self.video_dict[video_id], 'new_annotation.json')
        try:
            with open(annotations_file, 'r') as f:
                json_dict = json.load(f)
        except:
            print(f'DEBUGJM {annotations_file}')

        for i in range(k):
            ind = r_ind[i]
            start_, end_ = caption['start'][ind], caption['end'][ind]
            try:
                words = self.tokenizer.tokenize(json_dict["frames"][str(ind+1)]["caption"]["1"])
            except:
                words = self.tokenizer.tokenize("This is an action of something")
    ...
    def _get_rawvideo(self, idx, s, e):
        video_mask = np.zeros((len(s), self.max_frames), dtype=np.longlong)
        max_video_length = [0] * len(s)

        # Pair x L x T x 3 x H x W
        video = np.zeros((len(s), self.max_frames, 1, 3, 224, 224), dtype=np.float64)
    ...
    def __getitem__(self, feature_idx):
        # get video path from feature_idx

        # get annotation file from video path

        # extract description from annotation file
        video_id, sub_id = self.iter2video_pairs_dict[feature_idx]
        caption = self.caption_dict[video_id]

        pairs_text, pairs_mask, pairs_segment, starts, ends = self._get_text(video_id, caption, sub_id)
        # raise ValueError(video_id, sub_id, caption)
