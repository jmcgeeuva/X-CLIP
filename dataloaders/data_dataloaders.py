...
from dataloaders.dataloader_edu_retrieval import Edu_DataLoader
...
def dataloader_edu_train(args, tokenizer):
    edu_dataset = Edu_DataLoader(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(edu_dataset)
    dataloader = DataLoader(
        edu_dataset,
        batch_size=args.batch_size // args.n_gpu,
        num_workers=args.num_thread_reader,
    ...
    return dataloader, len(edu_dataset), train_sampler

def dataloader_edu_test(args, tokenizer, subset="test"):
    edu_testset = Edu_DataLoader(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
    ...
    return dataloader_edu, len(edu_testset)
...
DATALOADER_DICT["edu"] = {"train":dataloader_edu_train, "val":dataloader_edu_test, "test":dataloader_edu_test}
